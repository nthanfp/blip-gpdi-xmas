...
$(function () {
    ...
    function sendPush(title, body, url, cb) {
        setLoading(true)
        $.post('<?php echo site_url('activities/notification/send_custom'); ?>', pushData({ title: title, body: body, click_url: url }))
            .done(function (res) {
                    if (res.success) {
    ...
    $('#btnSendPush').on('click', function () {
        ...
        if (!title) { $('#pushTitle').focus(); return }
        if (!body) { $('#pushBody').focus(); return }
