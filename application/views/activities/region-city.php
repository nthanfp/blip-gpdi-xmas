...
<body class="sidebar-mini layout-fixed sidebar-collapse">
                    ...
                    <table class="table table-sm table-striped mb-0">
                        <thead>
                            <tr>
                                ...
                                <th class="text-nowrap sort-header" data-sort="mst_reg_cityid" role="button">ID <span
                                        class="sort-indicator" data-sort-indicator="mst_reg_cityid"><i
                                            class="fas fa-sort text-secondary"></i></span></th>
                                <th class="text-nowrap sort-header" data-sort="city_name" role="button">City Name <span
                                        class="sort-indicator" data-sort-indicator="city_name"><i
                                            class="fas fa-sort text-secondary"></i></span></th>
                                <th class="text-nowrap sort-header" data-sort="mst_reg_provinceid" role="button">Province <span
            ...
            <div class="modal-dialog modal-md modal-dialog-scrollable">
                <div class="modal-content">
                    <div class="modal-header">
                        <h6 class="modal-title" id="modalTitle">New City</h6>
    ...
    <script>
        $(function () {
            const endpoints = {
                list: '<?= site_url("activities/region_city/data_list") ?>',
                create: '<?= site_url("activities/region_city/data_new") ?>',
                edit: '<?= site_url("activities/region_city/data_edit") ?>',
                update: '<?= site_url("activities/region_city/data_update") ?>',
                delete: '<?= site_url("activities/region_city/data_delete") ?>',
                provinceOption: '<?= site_url("activities/region_city/data_province_option") ?>'
            }
            ...
            const renderRows = (rows) => {
                ...
                const html = rows.map((row) => `
                    <tr>
                        <td class="d-none">${escapeHtml(row.mst_reg_cityid)}</td>
                        <td class="text-nowrap">${escapeHtml(row.mst_reg_cityid)}</td>
                        <td class="text-nowrap">${escapeHtml(row.city_name)}</td>
                        <td class="text-nowrap">${escapeHtml(provinceMap[row.mst_reg_provinceid] || row.mst_reg_provinceid)}</td>
                        <td class="text-nowrap text-right">
                            <button type="button" class="btn btn-xs btn-outline-primary btn-edit" data-id="${escapeHtml(row.mst_reg_cityid)}">
                                <i class="fas fa-edit"></i>
                            </button>
                            <button type="button" class="btn btn-xs btn-outline-danger btn-delete" data-id="${escapeHtml(row.mst_reg_cityid)}" data-name="${escapeHtml(row.city_name)}">
            ...
            $(document).on('click', '.btn-edit', function () {
                ...
                $.ajax({
                    ...
                    success: function (res) {
                        if (!res.success) {
                            Swal.fire('Error', res.message || 'Failed to load city', 'error')
                            return
                        }
                        const data = res.data || {}
                ...
                Swal.fire({
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            if (!response.success) {
                                Swal.fire('Error', response.message || 'Failed to delete city', 'error')
                                return
                            }
                            Swal.fire('Success', response.message || 'City deleted successfully', 'success')
                            loadData()
                        },
                        error: function (xhr) {
                            console.error(xhr.responseText);
            ...
            $('#btnSave').on('click', function () {
                const payload = {
                    id: fields.id.val(),
                    mst_reg_provinceid: fields.mst_reg_provinceid.val(),
                    city_name: fields.city_name.val().trim()
                }

                if (!payload.mst_reg_provinceid) {
                ...
                $.ajax({
                    ...
                    success: function (res) {
                        if (!res.success) {
                            Swal.fire('Error', res.message || 'Failed to save city', 'error')
                            return
                        }
